...
// --- FONCTION DE MÉLANGE (ALGORITHME FISHER-YATES) ---
...
function majInterfaceStatut(isLoaded) {
    ...
    if (statusEl) {
        statusEl.innerHTML = isLoaded ? `✅ ${listeAmisActive.length} amis chargés` : "⚠️ Liste vide";
...
async function lancerAssistant(nbAmisParCom, vitesse) {
    ...
    // --- MÉLANGE DE LA LISTE AU DÉBUT ---
    console.log("🔀 Mélange de la liste pour ce concours...");
    listeAmisActive = melangerListe([...listeAmisActive]);
...
if (!document.getElementById("helper-container")) {
    ...
    container.innerHTML = `
        ...
        <input type="number" id="nbAmis" value="3" min="1" style="padding:5px; border-radius:5px; border:1px solid #ccc; background: white !important; color: black !important; width: 100%;">
        <label style="font-size:11px; color: #000 !important; font-weight:bold;">Rapidité :</label>
        <select id="selectVitesse" style="padding:5px; border-radius:5px; border:1px solid #ccc; background: white !important; color: black !important; width: 100%;">
            <option value="Lent">Lent</option>
            <option value="Moyen" selected>Moyen</option>
            <option value="Rapide">Rapide</option>
            <option value="Extreme">Extreme ⚡</option>
        </select>
        <button id="startHelper" style="padding:10px; background:#1877F2 !important; color:white !important; border:none; border-radius:5px; cursor:pointer; font-weight:bold; margin-top:5px;">LANCER</button>
        <button id="stopHelper" style="display:none; padding:8px; background:#e74c3c !important; color:white !important; border:none; border-radius:5px; cursor:pointer; font-weight:bold;">⏹ ARRÊTER</button>
    `;
    ...
    document.getElementById("fileInput").onchange = (e) => {
        ...
        reader.onload = (e) => {
            ...
            chrome.storage.local.set({ maListeSauvegardee: liste }, () => {
                listeAmisActive = liste; majInterfaceStatut(true); alert("Liste mémorisée !");
            });
        };
